...
int main(int argc, char** argv) {
    ...
    PortValueMessageRegistry::getInstance().registerCustomCreator(
        "iso.example.irpsystem.irpdomain.ImmutableDeliverySchedule",
        [](nlohmann::json jv) {
            return new PortValueMessage<IrpEvent *, std::string>(
                jv.at("portName").get<std::string>(),
                "iso.example.irpsystem.irpdomain.ImmutableDeliverySchedule",
    ...
    const Coordinate manufacturerCoord(irp_data.manufacturer.x, irp_data.manufacturer.y);
    ...
    const FacilityProps facility_props(manufacturerCoord, startingInventory, inventoryCostRate);
    ...
    auto* manufacturerStreamingSimulator =
        new ManufacturerStreamingSimulator(kafkaCoordinator, devs_message::LONG_SIM_TIME,
            "manufacturer", manufacturer);

    auto* kafkaSimulatorRouter = new KafkaSimulatorRouter(props, receiverTopic, devsMessageFactory);
    kafkaSimulatorRouter->registerSimulator("manufacturer", manufacturerStreamingSimulator);
